  ...
  return (
    <div className="page"><div className="container" style={{maxWidth:700}}>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:12,marginBottom:24}}>
        {cart.map(item => (
          <div key={item._id} className="glass-card" style={{padding:'16px 20px',display:'flex',alignItems:'center',gap:16}}>
            ...
            <div style={{flex:1}}>
              <h4 style={{fontSize:15,fontWeight:600,color:'#f1f5f9'}}>{item.name}</h4>
            ...
            <div style={{display:'flex',alignItems:'center',gap:8}}>
              ...
              <span style={{fontSize:16,fontWeight:700,color:'#f1f5f9',minWidth:24,textAlign:'center'}}>{item.quantity}</span>
              <button onClick={()=>updateQuantity(item._id, item.quantity+1)} className="btn btn-secondary btn-sm" style={{padding:'4px 8px',minWidth:0}}>
                <Plus size={14} />
              </button>
            ...
            <span style={{fontSize:15,fontWeight:700,color:'#f1f5f9',minWidth:70,textAlign:'right'}}>Rs. {item.price * item.quantity}</span>
          </div>
        ))}
      </div>
      ...
      <div className="glass-card" style={{padding:24,marginBottom:16}}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}><span style={{color:'#94a3b8'}}>Subtotal</span><span style={{color:'#f1f5f9'}}>Rs. {cartTotal}</span></div>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}><span style={{color:'#94a3b8'}}>Delivery Fee</span><span style={{color:'#22c55e'}}>FREE</span></div>
        <div style={{height:1,background:'rgba(255,255,255,0.1)',margin:'12px 0'}}></div>
        <div style={{display:'flex',justifyContent:'space-between'}}><span style={{fontSize:18,fontWeight:700,color:'#f1f5f9'}}>Total</span><span style={{fontSize:18,fontWeight:800,color:'#ff6b35'}}>Rs. {cartTotal}</span></div>
      </div>
